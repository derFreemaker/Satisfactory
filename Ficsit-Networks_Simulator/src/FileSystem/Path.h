...
namespace CodersFileSystem
{
    ...
    public:
        ...
        Path(std::string str)
        {
            ...
            if (str.length() < 1 && path.size() > 0 && path[path.size() - 1] != '/')
                path += "/";
            else
                append(str);
        }
        ...
        bool startsWith(const Path &other) const
        {
            ...
            return path.substr(0, o.path.size()) == o.getPathString();
        }
        ...
        Path normalize() const
        {
            ...
            do
            {
                ...
                else if (node == "..")
                {
                    ...
                    if (newPath.path.size() < 1 && isAbsolute())
                        newPath.path = "/";
                }
                else if (isNode(node))
                {
        ...
        {
            return relative().getPathString() == other.relative().getPathString();
        }

        bool operator<(const Path &other) const
        {
            return getPathString() < other.getPathString();
        }
